...
experimentname = 'A1_B2_C1_1shot_sun'

for test_epoch in range(1):
	model = GCNMetaModel(myconfig)
	saver = tf.train.Saver(tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES), max_to_keep=5)
	log_to_save = []
	...
	for epoch in range(60000):
		...
		if val_acc > best_val_acc:
			saver.save(model.sess, './models/'+ experimentname + '-' + str(epoch))
			print("  (Best epoch. acc increased to %.5f from %.5f.)" % (val_acc, best_val_acc))
