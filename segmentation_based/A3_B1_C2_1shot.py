...
class GCNMetaModel(object):
    ...
    def prepare_specific_graph_model(self):
        ...
        self.placeholders['target_x'] = tf.placeholder(tf.float32,[5,224,224,3],name='target_x')
        self.placeholders['target_roi'] = tf.placeholder(tf.float32,[5,self.params['n_cluster'],6],name='target_roi')
        self.placeholders['target_adj'] = tf.placeholder(tf.float32,[5,self.params['n_cluster']+1,self.params['n_cluster']+1,self.params['edge_dims']],name='target_adj')
        self.placeholders['num_vertices'] = tf.placeholder(tf.int32, None)


        self.placeholders['support_label'] = tf.placeholder(tf.int32, [5])
        self.placeholders['target_label'] = tf.placeholder(tf.int32, [5])
        ...
        self.placeholders['target_v'] = tf.placeholder(tf.int32, [5])
    ...
    def compute_final_node_representations_v16(self, node, edge, grad, name):
        ...
        self.weights['proj_h_sim1'] = fc(e, 1, name + '_proj_h_sim1', with_bias=False, relu=False, reuse=tf.AUTO_REUSE)
        if grad is not None:
    ...
    def get_data_batch(self, is_training):
        ...
        for batch in range(1):
            ...
            for k,classes in enumerate(selected_classes):
                ...
                selected_samples = np.random.choice(len(dirs), 1+1, False)
