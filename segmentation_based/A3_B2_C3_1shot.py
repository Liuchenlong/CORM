...
class GCNMetaModel(object):
	...
	def __init__(self, myconfig):
		...
		#self.roi_dir = self.train_dir + 'segmentation_based/sunrois30v3'
		self.roi_dir = self.train_dir + 'segmentation_based/rois30'
	...
	def prepare_specific_graph_model(self):
		...
		self.placeholders['target_x'] = tf.placeholder(tf.float32,[self.meta_batch_size,25,imgsize,imgsize,3],name='target_x')
		self.placeholders['target_roi'] = tf.placeholder(tf.float32,[self.meta_batch_size,25,self.params['n_cluster'],6],name='target_roi')
		...
		self.placeholders['target_label'] = tf.placeholder(tf.int32, [self.meta_batch_size,25])
		self.placeholders['support_v'] = tf.placeholder(tf.int32, [self.meta_batch_size,5])
		self.placeholders['target_v'] = tf.placeholder(tf.int32, [self.meta_batch_size,25])
	...
	def construct_conv_weights(self):
		...
		weights['w5'] = tf.get_variable('w5', [self.params['in_size'], int(self.params['in_size']/2)], initializer=fc_initializer,)
	...
	def forward_conv(self, input, weights, rois, reuse=tf.AUTO_REUSE, scope=''):
		...
		meta_batch_size = rois.shape.as_list()[0]
		roi_pool5 = roi_pool_layer(hidden4, rois, 7, meta_batch_size, name='ROIPooling')
		roi_conv = global_pool(roi_pool5, name='roi_global_pool', ptype='max')
		roi_conv = tf.reshape(tf.squeeze(roi_conv), [meta_batch_size, num_bbox, -1])
		...
		ph_dim = int(in_dim / 2)
		print(h.shape)
		ph = tf.matmul(tf.reshape(h,[-1, in_dim]), weights['w5'])
		print(ph.shape)
	...
	def task_metalearn(self, inp, reuse=tf.AUTO_REUSE):
		...
		inputa, inputb, roia, roib, labela, labelb = inp
		task_outputbs, task_lossesb = [], []
		num_updates = self.num_updates
		task_accuraciesb = []
		task_outputa,local_output = self.forward(inputa, self.weights, roia, reuse=reuse)  # only reuse on the first iter
		...
		output,local_output = self.forward(inputb, fast_weights, roib, reuse=True)
		...
		for j in range(num_updates - 1):
			task_outputa,local_output = self.forward(inputa, fast_weights, roia, reuse=reuse)
	...
	def make_model(self, prefix='metatrain_'):
		...
		with tf.variable_scope('model', reuse=tf.AUTO_REUSE) as training_scope:
			...
			unused = self.task_metalearn((inputa[0], inputb[0], roia[0], roib[0], labela[0], labelb[0]), tf.AUTO_REUSE)
			out_dtype = [tf.float32, [tf.float32]*num_updates, tf.float32, [tf.float32]*num_updates]
			out_dtype.extend([tf.float32, [tf.float32]*num_updates])
			result = tf.map_fn(self.task_metalearn, elems=(inputa, inputb, roia, roib, labela, labelb), dtype=out_dtype, parallel_iterations=self.meta_batch_size)
	...
	def get_data_batch(self, is_training):
		...
		for batch in range(self.meta_batch_size):
			...
			for k,classes in enumerate(selected_classes):
				...
				selected_samples = np.random.choice(len(dirs), 1+5, False)
				selected_roi = [roi_dirs[i] for i in selected_samples]
				selected_img = [img_dirs[i] for i in selected_samples]
				roi_dirs = [np.load(dir)/224.0*imgsize for dir in selected_roi]
				img_dirs = [np.array(PiLImageResize(filenameToPILImage(dir)))*1.0/255.0 for dir in selected_img]
				support_x.extend([(img_dirs[i], roi_dirs[i], k) for i in range(len(roi_dirs))[:1]])
				target_set.extend([(img_dirs[i], roi_dirs[i], k) for i in range(len(roi_dirs))[1:]])
				np.random.shuffle(support_x)
			support_img,support_roi,support_y = zip(*support_x)
			support_set.append((support_img, support_roi, support_y))
		support_img,support_roi,support_y = zip(*support_set)
		...
		support = {'img':support_img, 'roi':support_roi}
		target_img,target_roi,target_y = zip(*target_set)
		target_img = np.reshape(np.array(target_img), (self.meta_batch_size, 25, imgsize, imgsize, 3))
		target_roi = np.reshape(np.array(target_roi), (self.meta_batch_size, 25, self.params['n_cluster'], 6))
		target_set_y = np.reshape(np.array(target_y), (self.meta_batch_size, 25))
		target = {'img':target_img, 'roi':target_roi}
