...
class ResNet(object):
    ...
    def __init__(self, model_flags):
        ...
        self.inkernels = model_flags.inkernels
        self.expansion = model_flags.expansion
    ...
    def inference(self, X, is_training):
        ...
        x = conv(X, 7, 64, 2, pad=3, name='conv1')
        x = BatchNorm(x, is_training, name='bn1')
        x = Relu(x)
        print(x.shape)
        ...
        # ResLayer 1 in (56x56)
        x = self.ResLayer(x, self.nlayers[0], 64, 1, is_training, name='layer1')
        print(x.shape)

        # ResLayer 2 in (56x56)
        x = self.ResLayer(x, self.nlayers[1], 128, 2, is_training, name='layer2')
        print(x.shape)

        # ResLayer 3 in (28x28)
        x = self.ResLayer(x, self.nlayers[2], 256, 2, is_training, name='layer3')
        print(x.shape)

        # ResLayer 4 in (14x14)
        x = self.ResLayer(x, self.nlayers[3], 512, 2, is_training, name='layer4')
        L = x.shape.as_list()[1]
        x = avgpool(x, L, 1, padding='VALID')
        print(x.shape)
        ox = tf.squeeze(x, [1, 2])
        print(ox.shape)

        # classifier
        x = fc(ox, self.num_classes, name='fc5')

        return ox

    def ResLayer(self, x, nlayers, kernels, stride, is_training, name):
        with_downsample = False
        with tf.variable_scope(name):
            if stride != 1 or self.inkernels != kernels * self.expansion:
                with_downsample = True
            x = self.Bottleneck(x, kernels, stride, with_downsample, is_training, name='0')
            self.inkernels = kernels * self.expansion
            for i in range(1, nlayers):
                x = self.Bottleneck(x, kernels, 1, False, is_training, name=str(i))

        return x

    def Bottleneck(self, x, kernels, stride, with_downsample, is_training, name):
        res = tf.identity(x)
        with tf.variable_scope(name):
            nx = conv(x, 1, kernels, 1, name='conv1')
            nx = BatchNorm(nx, is_training, name='bn1')
            nx = Relu(nx)
            nx = conv(nx, 3, kernels, stride, name='conv2', pad=1)
            nx = BatchNorm(nx, is_training, name='bn2')
            nx = Relu(nx)
            nx = conv(nx, 1, kernels * self.expansion, 1, name='conv3')
            nx = BatchNorm(nx, is_training, name='bn3')
            if with_downsample:
                res = self.downsample(x, kernels, stride, is_training)
            nx += res
            nx = Relu(nx)

        return nx

    def downsample(self, x, kernels, stride, is_training, name='downsample'):
        with tf.variable_scope(name):
            x = conv(x, 1, kernels * self.expansion, stride, 'conv')
            x = BatchNorm(x, is_training, 'bn')
...
def BatchNorm(x, is_training, name):
    # decay=0.9, epsilon=1e-5, for pytorch BatchNorm2d default parameters
