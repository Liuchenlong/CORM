...
class GCNMetaModel(object):
    ...
    def gated_regression_v6(self, last_h, regression_node, regression_transform):
        ...
        last_h = tf.reshape(last_h[:, 0, :], [-1, self.params['out_size']])
    ...
    def compute_final_node_representations_v16(self, node, grad, name):
        v = self.placeholders['num_vertices']
        h_dim = self.params['hidden_size']
        e_dim = self.params['edge_dims']
        o_dim = self.params['out_size']

        h = tf.nn.l2_normalize(node, axis=-1)
        self.norm_init_h = h
        ...
        ph_dim = int(in_dim / 2)
        self.weights['proj_h_sim1'] = fc(tf.reshape(h, [-1, in_dim]), ph_dim, name + '_proj_h_sim1', with_bias=False, relu=False, reuse=tf.AUTO_REUSE)
        if grad is not None:
            self.weights['proj_h_sim1'] += grad['proj_h_sim1']
        ph = tf.reshape(self.weights['proj_h_sim1'], [-1, v, ph_dim])
        ph = tf.nn.l2_normalize(ph, axis=-1)
    ...
    def get_grad(self):
        ...
        for i in range(support_n):
            ...
            support_representation = self.compute_final_node_representations_v16(support_i, None, name = 'base')
    ...
    def make_model(self):
        ...
        for i in range(target_n):
            ...
            num = self.placeholders['num_vertices']*(int(self.params['in_size']/2))
            csn = {
                'proj_h_sim1': tf.reshape(target_meta1[0][:num],[self.placeholders['num_vertices'],int(self.params['in_size']/2)]),
                'gcn_0_sim': tf.reshape(target_meta1[0][num:],[self.placeholders['num_vertices'],self.params['hidden_size']])
            }
            target_representation = self.compute_final_node_representations_v16(target_i[None,:,:], csn, name = 'base')
